...
- ENTITY_CODE:          "produzioni_attive"
- SOURCE_IDENTITY_KEY:  "id_dettaglio"  (= ID_DETTAGLIO in DPRE_PROD/EasyJob)
...
from nssp_v2.sync.produzioni_attive.source import ProduzioneAttivaSourceAdapter
from nssp_v2.sync.contract import RunMetadata


class ProduzioneAttivaSyncUnit:
    """Sync unit per l'entita `produzioni_attive`.

    Responsabilita:
    - acquisisce produzioni attive dalla sorgente DPRE_PROD (read-only)
    - allinea il target interno sync_produzioni_attive via upsert
    ...
    def run(self, session: Session, source: ProduzioneAttivaSourceAdapter) -> RunMetadata:
        """Esegue la sync `produzioni_attive`.
        ...
        Args:
            ...
            source:   adapter read-only per la sorgente produzioni attive
        ...
        try:
            ...
            # Carica i record gia presenti nel target interno
            existing: dict[int, SyncProduzioneAttiva] = {
                obj.id_dettaglio: obj
                for obj in session.query(SyncProduzioneAttiva).all()
            }
            ...
            # Delete handling: mark_inactive per id non piu presenti in sorgente

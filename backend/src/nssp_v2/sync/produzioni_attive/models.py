"""
Modelli SQLAlchemy per il target interno sync `produzioni_attive`.

Ownership: sync layer — solo le sync unit scrivono qui (DL-ARCH-V2-009 §1).

Tabelle di questa entita:
- sync_produzioni_attive: mirror interno di DPRE_PROD (EasyJob) — owned da questa sync unit

Campi allineati a EASY_PRODUZIONI.md (TASK-V2-028):
...
class SyncProduzioneAttiva(Base):
    """Mirror interno di DPRE_PROD (EasyJob).

    Source identity key: id_dettaglio (ID_DETTAGLIO in DPRE_PROD).
    Delete handling: mark_inactive (attivo=False) — no hard delete.
    """

    __tablename__ = "sync_produzioni_attive"
